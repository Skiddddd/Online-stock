...
1. Install dependencies:
   `npm install`
2. Set `VITE_API_BASE_URL` in [.env.local](.env.local) to your backend URL (optional for local dev; defaults to `http://localhost:4000`)
3. Run the app:
   `npm run dev`
...
- `GEMINI_API_KEY` (server-side Gemini key used by `/api/ai/*` endpoints)
