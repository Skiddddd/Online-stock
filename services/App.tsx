            ...
            <form onSubmit={handleAuth} className="space-y-4">
              ...
              >
                {loading ? (
                  ...
                  <span>{isRegistering ? 'Start Investing' : 'Login Dashboard'}</span>
                )}
              </button>
            </form>
      ...
      {state.currentUser.role === UserRole.ADMIN && (
        <>
          {activeTab === 'admin-overview' && (
            <AdminView users={state.users} transactions={state.transactions} onUpdateStatus={handleUpdateStatus} systemConfig={state.systemConfig} onUpdateConfig={handleUpdateConfig} view="overview" />
          )}
          {activeTab === 'admin-users' && (
            <AdminView users={state.users} transactions={state.transactions} onUpdateStatus={handleUpdateStatus} systemConfig={state.systemConfig} onUpdateConfig={handleUpdateConfig} view="users" />
          )}
          {activeTab === 'admin-tx' && (
            <AdminView users={state.users} transactions={state.transactions} onUpdateStatus={handleUpdateStatus} systemConfig={state.systemConfig} onUpdateConfig={handleUpdateConfig} view="overview" />
          )}
          {activeTab === 'admin-settings' && (
            <AdminView users={state.users} transactions={state.transactions} onUpdateStatus={handleUpdateStatus} systemConfig={state.systemConfig} onUpdateConfig={handleUpdateConfig} view="settings" />
          )}
        </>
      )}
